...
VALORG="valgrind -q"  # Si valgrind no está instalado, quitar "valgrind -q" de esta linea (quedaría VAL=""). Ojo: La práctica debe probarse siempre con valgrind antes de la entrega.
npruebasa=$(ls -1 pruebas/*.txt|wc -l)
mata=./mata
comparefiles=./comparefiles
cmpBinFiles=./cmpBinAgency
...
function prepArgsBin ()
{
  ...
  if test "$1" == "08_saveData" ; then
    VAL="" # desactivar valgrind para esta prueba
  elif test "$1" == "09_argument-i" ; then
    args="-i pruebas/fileImportFromCSV.csv"
  elif test "$1" == "10_argument_error" ; then
    args="-l "
  fi
}
  ...
  if test -s $tempfile; then
     ...
     if [ $? -ne 0 ]; then
       diff -EwB $OBT/${bn}.salida-obtenida $SAL/${bn}.salida-esperada 2>&1
        ...
        if test "$bn" == "06_exportToCSV"; then
 	        $comparefiles $OBT/06_fileExportToCSV.csv $SAL/06_fileExportToCSV.csv
	        if [ $? -ne 0 ]; then
		        diff -EwB $OBT/06_fileExportToCSV.csv $SAL/06_fileExportToCSV.csv 2>&1
	        else
 	          echo "OK"
	          let pok++
	        fi           
	elif test "$bn" == "08_saveData"; then
 	        $VAL $cmpBinFiles $OBT/08_fileSaveData.bin $SAL/08_fileSaveData.bin 2>&1
		if [ $? -eq 0 ]; then
 		          echo "OK"
		          let pok++
 		fi
